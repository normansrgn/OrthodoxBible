...
// --- ДАННЫЕ БИБЛИИ ---
let bibleData = [];
try { bibleData = JSON.parse(fs.readFileSync('./bible.json', 'utf8')).Books; } catch (e) { console.error("Ошибка загрузки bible.json"); }
...
function getDetailedCalendar() {
    ...
    // Исправлено: ссылка теперь в обратных кавычках
    ...
    // Исправлено: весь текст обернут в одни обратные кавычки для удобства и корректности
    const messageText = `<b>📅 ЦЕРКОВНЫЙ КАЛЕНДАРЬ</b>
...
async function createVerseCard(text, ref) {
    ...
    ctx.fillStyle = '#f4e7d3'; ctx.textAlign = 'center'; ctx.font = 'italic 44px Georgia';
    const wrapText = (t) => {
    ...
    lines.forEach((l, i) => ctx.fillText(l, 540, startY + (i * 80)));
    ctx.fillStyle = '#c5a059'; ctx.font = 'bold 50px Georgia';
    ctx.fillText(ref.toUpperCase(), 540, startY + (lines.length * 80) + 120);
...
bot.start((ctx) => {
    ...
    ctx.replyWithHTML(`<b>Мир дому твоему!</b> ☦️\n\nДобро пожаловать в духовный помощник.`, mainReplyMenu);
});
...
bot.hears('📅 Календарь', (ctx) => {
    ...
    ctx.replyWithHTML(cal.text, { link_preview_options: { is_disabled: false, prefer_large_media: true }, ...Markup.inlineKeyboard([[Markup.button.webApp('☦️ Открыть Азбуку Веры', cal.link)], [Markup.button.callback('📖 Читать Евангелие дня', 'test_new')]]) });
});
...
bot.hears('🙏 Молитвослов', (ctx) => {
    ...
    ctx.replyWithHTML(text, {
        link_preview_options: {
            url: 'https://azbyka.ru/molitvoslov/', // Явное указание ссылки для красивого превью
            is_disabled: false,
...
bot.hears('📂 Псалтирь на потребу', (ctx) => {
    ...
    ctx.replyWithHTML(`<b>Псалтирь на всякую потребу</b>\n┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\nВыберите категорию:`, Markup.inlineKeyboard(buttons));
});

bot.hears('🔖 Закладка', (ctx) => {
    const b = db[ctx.chat.id]?.bookmark;
    if (b) return sendChapter(ctx, b.bId, b.cId, false);
    ctx.replyWithHTML(`<b>🔖 ЗАКЛАДКИ</b>\n🕊━━━━━━━━━━━━━━━━🕊\n\nПрочитанная глава появится здесь автоматически.`);
});
...
bot.on('text', (ctx) => {
    ...
    let txt = `🔎 <b>РЕЗУЛЬТАТЫ ПОИСКА:</b>\n🕊━━━━━━━━━━━━━━━━🕊\n\n`;
    let btns = results.map((r, i) => { txt += `<b>${i + 1}. ${r.ref}</b>\n<i>${r.text}</i>\n\n`; return Markup.button.callback(`${i + 1} 📖`, `read_new_${r.bId}_${r.cId}`); });
    ctx.replyWithHTML(txt, Markup.inlineKeyboard([btns, [Markup.button.callback('🏠 Меню', 'start_over')]]));
});
...
bot.action(/ps_cat_(\d+)/, (ctx) => {
    ...
    ctx.editMessageText(`<b>${cat.name}</b>\nВыберите псалом:`, { parse_mode: 'HTML', ...Markup.inlineKeyboard(rows) });
});

bot.action('back_to_ps_cats', (ctx) => {
    const buttons = PSALMS_CATEGORIES.map((cat, idx) => [Markup.button.callback(cat.name, `ps_cat_${idx}`)]);
    ctx.editMessageText(`<b>Псалтирь на всякую потребу</b>\nВыберите категорию:`, { parse_mode: 'HTML', ...Markup.inlineKeyboard(buttons) });
});

bot.action('start_over', (ctx) => { ctx.editMessageText('<b>📚 РАЗДЕЛЫ ПИСАНИЯ</b>', { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('📜 Ветхий Завет', 'test_old'), Markup.button.callback('📖 Новый Завет', 'test_new')]]) }).catch(() => { }); });
...
bot.action(/bk_(\d+)/, (ctx) => {
    ...
    rows.push([Markup.button.callback('⬅️ К списку книг', bId >= 40 ? 'test_new' : 'test_old')]);
    ctx.editMessageText(`<b>📖 ${getBookName(bId)}</b>\nВыберите главу:`, { parse_mode: 'HTML', ...Markup.inlineKeyboard(rows) });
});
...
bot.launch().then(() => console.log('☦️ Бот запущен без ошибок'));